...
class Jobd
  ...
  def self.status_local; end

  def self.status_registering; end

  def self.status_running; end

  def self.status_queued; end

  def self.kill_max_time; end

  def self.command_kill; end
  ...
  def self.my_json(list)
    ...
    j = {}
    j['ref'] = list['ref']
    j['position'] = if !(list['position'])
                      '_POSITION_'
                    else
                      list['position']
                    end

    j['type'] = (list['type'] || 'head')
    ...
    if list['timeline']
      ...
      list['timeline'].sort_by { |k, _v| k }.each do |list_timeline|
    ...
    j['topology'] = list['topology'] if list['topology'] # #splaynet
  ...
  def self.head_list(job, m_s_s)
    my_json(raw_list(job, m_s_s, job[:list_size])) # a string now...
  end
  ...
  def self.random_lists(job, m_s_s)
    ...
    size = job_list['nodes'].size - 1 if size > job_list['nodes'].size - 1
  ...
  def self.send_all_list(job, selected_query)
    ...
    when 'HEAD' # simple head list of job['list_size'] element
      ...
      pos = 1
      m_s_s.each do |m_s|
        q_act += "('#{m_s[:splayd_id]}','#{job[:id]}','LIST','#{pos}','TEMP'),"
        pos += 1
      end
      if q_act != ''
  ...
  def self.send_start(job, query)
    ...
    $db[query].each do |m_s|
      q_act += "('#{m_s[:splayd_id]}','#{job[:id]}','START', '#{job[:ref]}'),"
    end
    if q_act != ''
      q_act = q_act[0, q_act.length - 1]
      # puts "ACTIONS"
  ...
  def self.create_filter_query(job)
    ...
    distance_filter = ''
    if job[:distance] && job[:latitude] && job[:longitude]
      distance_filter =
  ...
  def self.countries_by_continent
    countries = {}
    countries['af'] = %w[ao bf bi bj bw cd cf cg ci cm
  ...
  def self.select_splayds(job)
    ...
    $db[filter_query].each do |m|
      next unless (m[:network_send_speed] / c_splayd['max_number'][m[:id]] >= job[:network_send_speed]) &&
    ...
    if occupation.size < job[:nb_splayds]
      ...
      no_resources = true if nb_total < job[:nb_splayds]
    ...
    [c_splayd, occupation, status_msg, normal_ok, mandatory_ok, designated_ok, no_resources, false]
  end

  # Splayds selection for JobdStandard and JobdTrace
  def self.status_local_common(job)
    c_splayd, occupation, status_msg, normal_ok, _mandatory_ok, designated_ok, no_resources, do_next = select_splayds(job)
    return c_splayd, occupation, 0, nil, true if do_next == true
    ...
    if (!normal_ok || !designated_ok) && !no_resources
      if job[:strict] == 'FALSE'
        set_job_status(job[:id], 'QUEUED', status_msg)
        return c_splayd, occupation, 0, nil, true
      else
    ...
    [c_splayd, occupation, nb_selected_splayds, new_job, false]
  end
  ...
  def self.status_queued_common(job)
    ...
    if queue_timeout != 0 && queue_timeout < @@max_queue_timeout
      # take into account user-defined timeout
      if Time.now.to_i > status_time + queue_timeout
        set_job_status(job[:id], 'QUEUE_TIMEOUT')
      end
    else
      # take into account administrator-defined timeout
      if Time.now.to_i > status_time + @@max_queue_timeout
        set_job_status(job[:id], 'QUEUE_TIMEOUT')
      end
    end
