...
class Containers::Heap
  ...
  def consolidate
    ...
    degrees = []
    roots.each do |new_root|
      min = new_root if @compare_fn[new_root.key, min.key]
      # check if we need to merge
      if degrees[new_root.degree].nil? # no other node with the same degree
        degrees[new_root.degree] = new_root
        next
      else # there is another node with the same degree, consolidate them
        degree = new_root.degree
        until degrees[degree].nil?
          other_root_with_degree = degrees[degree]
          if @compare_fn[new_root.key, other_root_with_degree.key] # determine which node is the parent, which one is the child
            smaller = new_root
        ...
        degrees[degree] = new_root
        min = new_root if min.key == new_root.key # this fixes a bug with duplicate keys not being in the right order
      end
    end
    @next = min
  end
