...
class JobdTrace < Jobd
  ...
  def self.thread(job_params)
    @@threads[job_params['id']] = Thread.new(job_params) do |job|
      begin
        ...
        $db["SELECT splayd_id FROM splayd_selections
          ...
          q_act += "('#{splayd_id}','#{job['id']}','TRACE_END','#{job['ref']}','WAITING'),"
        end
        q_act = q_act[0, q_act.length - 1]

        $db.run("INSERT INTO actions (splayd_id, job_id, command, data, status) VALUES #{q_act}")
  ...
  def self.status_registering
    $db["SELECT * FROM jobs WHERE scheduler='#{@@scheduler}' AND status='REGISTERING'"].each do |job|
      ...
      if possible_splayds.size == scheduling.size
        ...
        start_nodes.each do |trace_number_n|
          $log.debug("TRACE #{job[:id]}: initial #{trace_number_n}")
          $db.run("UPDATE splayd_selections SET
							trace_status='RUNNING'
							WHERE
							trace_number='#{trace_number_n}' AND
							job_id='#{job[:id]}'")
        end

        unless start_nodes.empty?
